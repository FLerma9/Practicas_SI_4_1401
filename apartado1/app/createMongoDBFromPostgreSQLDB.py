...
def insertMostRelated(mycol):
    ...
    for peli in mycol.find():
        list = []
        for x in mycol.find({"$and":[{"_id": { "$ne": peli["_id"]} }, {"genres": { "$all": peli["genres"]}}]}, { "_id": 0, "title": 1, "year": 1}).sort("title").sort("Year").limit(10):
            list.append(x)
        ...
        for x in mycol.find({}, {"_id": 0, "title": 1, "year": 1, "genres":1}).sort("title").sort("Year"):
            ...
            for genre in x["genres"]:
                if genre in peli["genres"]:
                    count += 1
            # Si coinciden la mitad o mas sin llegar al 100% la aniadimos
            if count >= media and count < len(peli["genres"]):
